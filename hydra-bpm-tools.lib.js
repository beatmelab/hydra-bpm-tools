...
(function () {
  ...
  function setRateMultiplier(value) {
    ...
    renderHud();
    flashBpmText();
    console.log("[Hydra BPM Tools] rate =", rateMultiplier, "| effective =", window.bpm);
  }
  ...
  function forceInitialBpmOnce() {
    ...
    if (window.hydraSynth || typeof window.bpm === "number") {
      ...
      renderHud();
      console.log("[Hydra BPM Tools] initial bpm base =", baseBpm, "| rate =", rateMultiplier, "| effective =", window.bpm);
    }
  }
  ...
  async function evaluateCurrentSketch() {
    ...
    try {
      ...
      console.error("[Hydra BPM Tools] eval error", err);
    }
  }
  ...
  function callHush() {
    try {
      ...
      console.error("[Hydra BPM Tools] hush error", err);
    }
    return false;
  }
  ...
  function resync() {
    try {
      if (window.hydraSynth?.synth) {
        ...
        flashBeatVisualizer();
        lastBeatIndex = -1;
        console.log("[Hydra BPM Tools] hydraSynth.synth.time = 0");
        return;
      }
      ...
      if (window.hydraSynth) {
        ...
        flashBeatVisualizer();
        lastBeatIndex = -1;
        console.log("[Hydra BPM Tools] hydraSynth.time = 0");
        return;
      }

      console.warn("[Hydra BPM Tools] Hydra instance not found");
    } catch (err) {
      console.error("[Hydra BPM Tools] resync error", err);
    }
  }
  ...
  function install() {
    ...
    window.hydraBpmTools = {
      setBpm: setBaseBpm,
      getBpm: getCurrentBaseBpm,
      getEffectiveBpm: getCurrentEffectiveBpm,
